      ...
      (entries) => {
        entries.forEach((entry) => {
          ...
          if (entry.isIntersecting) {
            ...
            el.style.opacity = "0";
            // Reset offset based on which edge the tile exited from so the
            // re-entrance animation always matches the scroll direction.
    ...
    >
      {rects.map((rect, i) => {
        ...
        return (
          <figure
            key={photo.id}
            className="group absolute m-0 overflow-hidden rounded-2xl"
            style={{
              left: rect.x,
              top: rect.y,
              width: rect.w,
